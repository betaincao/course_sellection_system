...
class Student extends Base{
    //学生信息展示
    public function lst(){
        $student = \think\Db::table('system_student')->order('s_grade asc')->paginate(15);
        $page = $student->render();
    ...
    public function searchbynum(){
        if(request()->isPost()){
            ...
            $s_num = input('s_num');
            //dump($data);die();
            $student = db('student')->where('s_num',$s_num)->select();
            if($student){
                $this->assign('student',$student);
    ...
    public function searchbyclass(){
        if(request()->isPost()){
            ...
            // echo $s_class;
            // die();
            $student = db('student')->where('s_class|s_major','like',$s_class)->select();
            if($student){
                $this->assign('student',$student);
                return $this->fetch('searchlstnum');
            }else{
